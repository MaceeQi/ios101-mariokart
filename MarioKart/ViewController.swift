...
class ViewController: UIViewController,
    ...
    private func moveKart(using gestureRecognizer: UIPanGestureRecognizer) {
        ...
        // move the kart to that location
    ...
    override func viewDidAppear(_ animated: Bool) {
        // call super class implementation since we're overriding this from super class
        super.viewDidAppear(animated)
        // runStartingAnimationsAllAtOnce()
        
        // race karts after they've all finished getting to starting spots
        runStartingAnimationsOneByOne {
            // fire completion closure when all three karts have animated
        ...
        getKartReadyToRace(kart: kartView0) { // animate kartView0 first
            self.getKartReadyToRace(kart: self.kartView1) { //...then kartView1
                self.getKartReadyToRace(kart: self.kartView2) { //...then kartView2
                    // runStartingAnimationsByOne also takes in an optional completion closure
                    // we'll go ahead and execute that closure only after the three animations are done
                    // we'll use '?' since this argument is optional
                    completion?()
                }
            }
        }
